import BrandLockup from './BrandLockup'

export default function Footer() {
  return (
    <footer className="bg-gray-900 text-gray-300 pt-14 pb-6">
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">

          {/* Brand */}
          <div>
            <div className="mb-4">
              <BrandLockup footer />
            </div>
            <p className="text-sm leading-relaxed text-gray-400">
              Quang Minh Group HVAC chuyên cung cấp máy lạnh chính hãng và giải pháp HVAC toàn diện: tư vấn, thiết kế, thi công, bảo trì hệ thống điều hòa cho mọi công trình.
            </p>
            <div className="flex gap-3 mt-4">
              <a href="https://www.facebook.com/NppDienLanhQuangMinhGroup" target="_blank" rel="noreferrer"
                className="w-9 h-9 rounded-full bg-gray-700 hover:bg-primary flex items-center justify-center transition-colors text-sm">
                f
              </a>
              <a href="https://zalo.me/0397507766" target="_blank" rel="noreferrer"
                className="w-9 h-9 rounded-full bg-gray-700 hover:bg-primary flex items-center justify-center transition-colors text-xs font-bold">
                Z
              </a>
            </div>
          </div>

          {/* Dịch vụ */}
          <div>
            <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">Dịch Vụ</h4>
            <ul className="space-y-2 text-sm">
              <li className="flex items-start gap-1.5"><span className="text-primary mt-0.5">✓</span> Cung cấp máy lạnh chính hãng</li>
              <li className="flex items-start gap-1.5"><span className="text-primary mt-0.5">✓</span> Thi công lắp đặt chuẩn kỹ thuật HVAC</li>
              <li className="flex items-start gap-1.5"><span className="text-primary mt-0.5">✓</span> Bảo trì – Vệ sinh – Sửa chữa tận nơi</li>
              <li className="flex items-start gap-1.5"><span className="text-primary mt-0.5">✓</span> Tư vấn giải pháp điện lạnh miễn phí</li>
            </ul>
          </div>

          {/* Liên hệ */}
          <div>
            <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">Liên Hệ</h4>
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2">
                <span className="text-primary">📍</span>
                <span>3/49 đường Thủ Khoa Huân, Khu Phố Bình Thuận 1, TP. Thuận An, Bình Dương</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="text-primary">📞</span>
                <a href="tel:0938777888" className="hover:text-primary transition-colors">0938 777 888</a>
                <span className="text-gray-600"> / </span>
                <a href="tel:0397507766" className="hover:text-primary transition-colors">039 750 7766</a>
              </li>
              <li className="flex items-center gap-2">
                <span className="text-primary">✉️</span>
                <a href="mailto:quangminhgroup.hvac@gmail.com" className="hover:text-primary transition-colors">quangminhgroup.hvac@gmail.com</a>
              </li>
              <li className="flex items-center gap-2">
                <span className="text-primary">⏰</span>
                <span>7:30 – 20:00</span>
              </li>
            </ul>
          </div>

          {/* Bản đồ */}
          <div>
            <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">Bản Đồ</h4>
            <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d5263.3427395865865!2d106.71394409999999!3d10.9703684!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3174d750ab0a0609%3A0x4ef9499493e766ec!2zUXXDoCBU4bq3bmcgRG9hbmggTmdoaeG7h3AgUU1H!5e1!3m2!1svi!2s!4v1777992226362!5m2!1svi!2s"
              width="100%"
              height="220"
              style={{ border: 0, display: 'block', borderRadius: '8px' }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title="Bản đồ QMG HVAC"
            />
          </div>
        </div>

        <div className="border-t border-gray-700 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-500">
          <p>© {new Date().getFullYear()} QMG HVAC – Giải Pháp Điện Lạnh Toàn Diện. All rights reserved.</p>
          <p>Thiết kế & phát triển bởi <span className="text-primary">QMG Team</span></p>
        </div>
      </div>
    </footer>
  )
}
